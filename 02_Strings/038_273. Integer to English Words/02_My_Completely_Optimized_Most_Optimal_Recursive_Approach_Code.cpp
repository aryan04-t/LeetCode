...
public:
    ...
    void helperFunc(int num, int suffixCount, unordered_map<int, string> &wordsMap, const vector<string> &suffixVector, string &words){

        if(num == 0) return; 

        int currPart = num % 1000; 

        helperFunc(num / 1000, suffixCount+1, wordsMap, suffixVector, words); 
        ...
        if(currPartStr.length() == 3 || currPartStr.length() == 2){
    ...
    string numberToWords(int num) {
        ...
        helperFunc(num, suffixCount, wordsMap, suffixVector, words); 
